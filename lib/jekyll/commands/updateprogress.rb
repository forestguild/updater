                    ...
                    prog.command(:updateprogress) do |c|
                        ...
                        c.action do |args, options|
                            ...
                            rio = Net::HTTP.post(URI('https://raider.io/api/crawler/guilds'),
                                                 {"realmId": options['realm-id'],
                                                  "realm": options['realm'],
                                                  "region": options['region'],
                                                  "guild": options['guild'],
                                                  "numMembers": "0"}.to_json,
                                                  "Content-Type" => "application/json").body()
                            Jekyll.logger.info "Raider.IO: " + (JSON.parse(rio)['success'] == true ? "OK" : "FAIL")
                            ...
                            Jekyll.logger.info "WoWProgress: " + (JSON.parse(wp.body)['success'] == true ? "OK" : "FAIL")
                        end
                    end
                end
                def getConfig(options)
                    config = Jekyll.configuration({})
                    ['region', 'realm', 'realm-id', 'guild'].each do |key|
                        if (options.key? key) === false
                            if key == 'realm-id'
                                options[key] = config['wowupdater'][key]
                            else
                                options[key] = config['wowupdater'][key].to_s
                            end
                        end
                    end
                    options
